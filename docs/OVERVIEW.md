...
- **Effect detection:** Two paths. Primary: schema-only registration through `useMotionWorks()`, `data-motionworks`, or an `application/motionworks+json` block. Secondary: zero-config auto-detection of CSS motion via `document.getAnimations()` plus `animationstart`/`transitionrun` listeners — CSS `@keyframes` animations (including scroll-driven and pseudo-element ones, and entrance one-shots caught from page load), and CSS transitions — registered as selectable effects with editable duration, delay, and easing (see `SCHEMA.md` → "What Happens Without an Explicit Schema"). Motion that runs on a main-thread engine (Framer Motion, GSAP, react-spring, WebGL, custom rAF) does not appear in `document.getAnimations()` and is not auto-detected; it is surfaced through the adoption workflow instead.
