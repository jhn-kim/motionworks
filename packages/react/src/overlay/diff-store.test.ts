...
describe('DiffStore', () => {
  describe('recordChange', () => {
    it('captures the baseline as `from` on first record', () => {
      ...
      expect(store.getDiff('effect-a')).toEqual({ radius: { from: 100, to: 145 } });
    });
    ...
    it('preserves the original `from` across subsequent records', () => {
      ...
      expect(store.getDiff('effect-a')).toEqual({ radius: { from: 100, to: 200 } });
    });
    ...
    it('records a diff for a shallow difference in a spring value', () => {
      ...
      expect(store.getDiff('effect-a')).toEqual({ spring: { from: baseline, to: changed } });
    });
  });
  ...
  describe('reconcile — matrix branches', () => {
    ...
    it('preserved: new baseline equals `from` → diff kept, no unexpected flag', () => {
      ...
      expect(store.getDiff('effect-a')).toEqual({ radius: { from: 100, to: 165 } });
    ...
    it('deep equality is used against object-valued params', () => {
      ...
      const clean = store.reconcile('effect-a', { spring: { stiffness: 320, damping: 25 } });
    ...
    it('leaves untouched params alone', () => {
      ...
      expect(store.getDiff('effect-a')).toEqual({ strength: { from: 0.5, to: 0.9 } });
    });
  });
