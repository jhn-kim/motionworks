...
describe('nameFromKeyframes', () => {
  it('describes the dominant visual change in plain words', () => {
    ...
    expect(nameFromKeyframes([{ transform: 'scale(1)', opacity: 0.7 }, { transform: 'scale(1.7)' }])).toBe('Pulse');
    expect(nameFromKeyframes([{ transform: 'rotate(0deg)' }, { transform: 'rotate(360deg)' }])).toBe('Spin');
    expect(nameFromKeyframes([{ transform: 'translateY(0)' }, { transform: 'translateY(-8px)' }])).toBe('Drift');
