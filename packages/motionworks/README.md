...
This adds the MCP server entry to `.mcp.json`, installs `motionworks` in React projects, and writes the agent instruction stanza — each step confirmed before it runs, each skipped when already done. `--yes` skips confirmations, `--stanza-only` writes only the instructions.
...
Use the `motionworks/react` export in the running app.
