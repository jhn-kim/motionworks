...
describe('standalone', () => {
  it('mounts the overlay and exposes browser helpers globally', async () => {
    ...
    await act(async () => { root = standalone.mount({ daemonUrl: 'http://127.0.0.1:52340' }); });
    expect(document.querySelector('[data-motionworks-root]')).not.toBeNull();
    const global = window as typeof window & { MotionWorks?: { readParams?: unknown } };
    expect(global.MotionWorks?.readParams).toBeTypeOf('function');
    await act(async () => { root?.unmount(); });
    delete (document as unknown as { currentScript?: HTMLScriptElement }).currentScript;
