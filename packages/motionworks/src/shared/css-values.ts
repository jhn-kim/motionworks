import type { EasingCurveValue, GradientStop, ParameterType, PathPoint, SpringValue } from './types.js';
...
export function parseEasing(css: string): EasingCurveValue | null {
  ...
  const match = /^cubic-bezier\(\s*(-?(?:\d+\.?\d*|\.\d+))\s*,\s*(-?(?:\d+\.?\d*|\.\d+))\s*,\s*(-?(?:\d+\.?\d*|\.\d+))\s*,\s*(-?(?:\d+\.?\d*|\.\d+))\s*\)$/i.exec(css.trim());
...
const NUMERIC_TYPES = new Set<ParameterType>(['spatial-radius', 'spatial-strength', 'temporal-decay', 'temporal-response', 'stagger', 'duration', 'scalar']);
const UNSUPPORTED_UNITS = new Set(['rem', 'em', 'vw', 'vh', 'vmin', 'vmax', '%']);

export function defaultUnitFor(type: ParameterType, schemaUnit?: string): string {
...
function splitTopLevel(value: string, separator = ','): string[] {
  ...
  for (let index = 0; index < value.length; index++) {
    ...
    else if (char === separator && depth === 0) { result.push(value.slice(start, index).trim()); start = index + 1; }
  }
  result.push(value.slice(start).trim());
  return result;
}

export function decodeCssValue(type: ParameterType, css: string): { value: unknown; unit: string } | null {
  ...
  if (NUMERIC_TYPES.has(type)) {
    ...
    if ((type === 'duration' || type === 'stagger') && unit === 's') { value *= 1000; normalizedUnit = 's'; }
  ...
  if (type === 'spring-response') {
    ...
    if (parts.some((part) => !Number.isFinite(part)) || (parts.length !== 1 && parts.length !== 2 && parts.length !== 3)) return null;
    if (parts.length === 1) return { value: parts[0], unit: '' };
    return { value: { stiffness: parts[0], damping: parts[1], ...(parts[2] !== undefined && { mass: parts[2] }) }, unit: '' };
  }
  if (type === 'gradient') {
  ...
  if (type === 'path') {
    ...
    if (match === null) return null;
    const tokens = (match[2] ?? '').trim().match(/[MLC]|-?(?:\d+\.?\d*|\.\d+)/gi);
    if (tokens === null || tokens.join(' ').replace(/\s+/g, '') !== (match[2] ?? '').replace(/[ ,]+/g, '')) return null;
    ...
    while (index < tokens.length) {
      ...
      if (nums.length !== count || nums.some((n) => !Number.isFinite(n))) return null;
      index += count;
      if (command === 'C') points.push({ x: nums[4]!, y: nums[5]!, cp1: { x: nums[0]!, y: nums[1]! }, cp2: { x: nums[2]!, y: nums[3]! } });
...
export function encodeCssValue(type: ParameterType, value: unknown, unit: string): string {
  if (NUMERIC_TYPES.has(type)) {
    let number = value as number;
    if ((type === 'duration' || type === 'stagger') && unit === 's') number /= 1000;
  ...
  if (type === 'spring-response') {
    ...
    return [spring.stiffness, spring.damping, spring.mass].filter((part) => part !== undefined).map((part) => formatNumber(part!)).join(' ');
  }
  if (type === 'gradient') return (value as GradientStop[]).map((stop) => `${stop.color} ${formatNumber(stop.stop * 100)}%`).join(', ');
  if (type === 'path') {
...
export function cssValuesEqual(type: ParameterType, a: string, b: string): boolean {
