...
describe('checkDrift', () => {
  it('warns when no instruction file exists', async () => {
    ...
    expect(warning!).toContain('MotionWorks: agent instructions may be outdated.');
    expect(warning!).toContain('Installed: @motionworks/mcp@1.0.0');
  ...
  it('warns when the stanza is older than the installed package', async () => {
    await writeFile(join(cwd, 'CLAUDE.md'), `${renderStanza('0.9.0')}\n`, 'utf8');
    const warning = await checkDrift({ cwd, packageVersion: '1.0.0' });
    expect(warning).not.toBeNull();
    expect(warning!).toContain('Installed: @motionworks/mcp@1.0.0');
  ...
  it('does not warn when the stanza is newer than the installed package', async () => {
    await writeFile(join(cwd, 'CLAUDE.md'), `${renderStanza('1.5.0')}\n`, 'utf8');
    const warning = await checkDrift({ cwd, packageVersion: '1.0.0' });
    expect(warning).toBeNull();
  });
  ...
  it('accepts an AGENTS.md-only project with a current stanza', async () => {
    await writeFile(join(cwd, 'AGENTS.md'), `${renderStanza('1.0.0')}\n`, 'utf8');
    const warning = await checkDrift({ cwd, packageVersion: '1.0.0' });
    expect(warning).toBeNull();
  });
  ...
  it('lists both files when both are stale', async () => {
    await writeFile(join(cwd, 'CLAUDE.md'), `${renderStanza('0.8.0')}\n`, 'utf8');
    await writeFile(join(cwd, 'AGENTS.md'), `${renderStanza('0.9.0')}\n`, 'utf8');
    const warning = await checkDrift({ cwd, packageVersion: '1.0.0' });
    expect(warning).not.toBeNull();
