...
async function collectDownstream(
  ...
  return new Promise((resolve, reject) => {
    const timer = setTimeout(() => {
      ...
      reject(
        new Error(
          `Timed out waiting for downstream message after ${timeoutMs}ms`,
        ),
      );
    }, timeoutMs);
...
describe("motionworks integration", () => {
  ...
  it("motionworks_get_instructions returns the schema emission guide verbatim", async () => {
    const res = await client.callTool({
      name: "motionworks_get_instructions",
      arguments: {},
    });
  ...
  it("drives the overlay flow (register → select → change → commit×2), then MCP surfaces both changesets oldest-first and clear_changes pops only the first", async () => {
    ...
    send(ws, {
      type: "change",
      payload: { effectId: effect.id, param: "radius", value: 165 },
    });
    send(ws, {
      type: "change",
      payload: { effectId: effect.id, param: "trail", value: 0.4 },
    });
    ...
    send(ws, {
      ...
      payload: {
        effectId: effect.id,
        elementSelector: "#hero > div.liquid-wrapper",
        diffs: diffs1,
      },
    });
    ...
    expect(changes.changesets.map((c) => c.id)).toEqual([
      changesetIdA,
      changesetIdB,
    ]);
    ...
    const listRes = await client.callTool({
      name: "motionworks_list_effects",
      arguments: {},
    });
    const effects = (
      extractJson(listRes.content as ToolContent) as {
    ...
    const syncedPromise = collectDownstream(
      ws,
      (m) => m.type === "source-synced",
    );
    ...
    if (synced.type !== "source-synced")
      throw new Error("expected source-synced");
