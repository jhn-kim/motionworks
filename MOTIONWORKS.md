...
Prefer a semantic type over `scalar`. There is no boolean type: expose the continuous parameter whose zero disables the phenomenon. Give every parameter a short human `label`; use `min` and `max` to constrain numeric editing.
